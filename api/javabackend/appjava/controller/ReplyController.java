...
public class ReplyController {
    ...
    public ResponseEntity getByParentLink(@RequestHeader("Authorization") String bearer, @PathVariable String link) {
        ...
        if (repository.findByParentLink(link).getId() == null)
            return ResponseEntity.badRequest().body("Not found");

        Reply ans = repository.findByParentLink(link);

        if (!jwtService.getRole(bearer).equals("ROLE_ADMIN") && ans.getOwnerLogin() != jwtService.getSubject(bearer))
            return ResponseEntity.badRequest().body("You don't have permission for that");
        

        return ResponseEntity.ok(repository.findByParentLink(link));
    }
    ...
    @PostMapping
    public ResponseEntity post(@RequestHeader("Authorization") String bearer, @RequestBody Reply obj) {
    ...
    @PathVariable Long id, @RequestBody Reply obj) {

        try { jwtService.TokenVerified(bearer, this.role); }
        catch (Exception e) { return ResponseEntity.badRequest().body(e.getMessage()); }

        Reply existingObj = repository.findById(id).get(); 
