class DB {
    ...
    static async CreateRow(tableName) {
        await DB.CheckToken();
        return await DB.Request(`/api/${tableName}/new/`, 'GET', null, LocalDB.GetToken());
    }

    static async UpdateRow(tableName, row) {
        await DB.CheckToken();
        return await DB.Request(`/api/${tableName}/update/`, 'PUT', row, LocalDB.GetToken());
    }

    static async DeleteRow(tableName, id) {
        await DB.CheckToken();
        return await DB.Request(`/api/${tableName}/delete/${id}/`, 'DELETE', null, LocalDB.GetToken());
    }
